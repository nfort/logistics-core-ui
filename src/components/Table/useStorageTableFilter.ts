...
export function useStorageTableFilter(key: string) {
  const [globalFilter, setGlobalFilter] = useStorage<GlobalFilter>(`${key}__gf`);
  const [filters, setFilters] = useStorage<Filters>(`${key}__f`, initialValueFilter);
  const [sortBy, setSortBy] = useStorage<SortBy>(`${key}__sb`, initialValueSortBy);

  function updateTableFilter(globalFilter: GlobalFilter, filters: Filters, sortBy: SortBy) {
